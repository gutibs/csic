    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (!empty($errors)) {
            ...
            try {
                ...
                $stmt = $pdo->prepare("UPDATE usuarios SET password = :password, activo = 1, activation_token = NULL WHERE id = :usuario");
                $stmt->execute(['password' => $hashedPassword, 'usuario' => $usuario]);
                $mensaje = "La cuenta ha sido activada con éxito.<br>Por favor, dirijase al <a href='../ingreso.php'>Login</a>";
